...
def get_booking_stats(business_id, period):
    ...
    if period == "day":
        ...
        cur.execute("""
            SELECT COUNT(*), SUM(s.price), b.status
            FROM bookings b
            JOIN services s ON b.service_name = s.service_name AND b.business_id = s.business_id
            WHERE b.business_id = ? AND b.booking_date = ?
            GROUP BY b.status
...
def create_payment_link(user_id, business_id, amount_rub, plan, days):
    try:
        ...
        data = {
            ...
            "currency_type": "fiat",
            "fiat": "RUB",
            "accepted_assets": "USDT,BTC,TON",
            "description": f"Подписка на бот на {days} дней",
            "paid_btn_name": "openBot",
            "paid_btn_url": f"https://t.me/{BOT_USERNAME}",
            "payload": f"user_{user_id}_business_{business_id}_plan_{plan}",
            "expires_in": 3600
        }
...
def activate_subscription(business_id, days):
    ...
    cur.execute(
        "UPDATE businesses SET subscription_end=?, is_trial=0, notification_sent_3d=0, expired_notification_sent=0 WHERE id=?",
        (new_end_str, business_id))
    conn.commit()
    conn.close()
    return new_end_str
